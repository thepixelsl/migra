...
function adminPath(pathname) {
  return pathname === "/admin-termine" || pathname.startsWith("/admin-termine/")
    || pathname === "/api/admin" || pathname.startsWith("/api/admin/");
}

function adminDenied(configured, unavailable = false) {
  return new Response(configured && !unavailable ? "Anmeldung erforderlich." : "Admin-Anmeldung vorübergehend nicht verfügbar.", {
    status: configured && !unavailable ? 401 : 503,
    headers: { "Cache-Control": "no-store", "Content-Type": "text/plain; charset=utf-8", "X-Robots-Tag": "noindex, nofollow, noarchive" },
  });
}

function adminRedirect(location) {
  return new Response(null, { status: 303, headers: { "Cache-Control": "no-store", Location: location } });
}
...
function withBunnyHeaders(response, env, requestUrl) {
  ...
  if (adminAuthPath(pathname)) {
    ...
    // same-origin form proof while withholding referrers from other sites.
  ...
  if (
    ...
    const privateAdminPage = pathname.startsWith("/admin-termine") || adminAuthPath(pathname);
...
export async function createBunnyRuntime(options = {}) {
  ...
  const adminAuthentication = createAdminAuthentication({ env, database, now: options.now || Date.now });
  ...
  const server = createServer(async (nodeRequest, nodeResponse) => {
    try {
      ...
      if (adminAuthPath(url.pathname)) {
        const loginResponse = await adminAuthentication.handle(request, url);
      ...
      if (adminPath(url.pathname)) {
        const access = await adminAuthentication.authorize(request);
        if (!access.authorized) {
          const isAdminPage = url.pathname === "/admin-termine"
            || url.pathname.startsWith("/admin-termine/");
          const deniedResponse = access.configured && !access.unavailable && isAdminPage
            ? adminRedirect(new URL("/admin-login/?next=/admin-termine/", adminAuthentication.origin).href)
            : adminDenied(access.configured, access.unavailable);
          await writeNodeResponse(
            nodeResponse,
        ...
        if (!["GET", "HEAD", "OPTIONS"].includes(request.method) && !adminAuthentication.validWriteOrigin(request)) {
