...
const consentSection = numberedSection(
  ...
  "Cookies und Einwilligungen",
  `<p>Für technisch nicht erforderliche Statistik- und Marketingdienste holen wir vor dem Laden des jeweiligen Dienstes Ihre Einwilligung ein. Statistik und Marketing können getrennt gewählt werden. Zusätzlich können Sie Google Tag Manager, Google Analytics, Microsoft Clarity und Meta Pixel im Banner einzeln auswählen. Dort können Sie auch die eingesetzten Service-Gruppen, Services und Provider mit ihren jeweiligen Angaben einsehen. Eine Ablehnung hat keinen Einfluss auf die grundlegende Nutzung dieser Website.</p>
...
<p>Google Tag Manager, Google Analytics, Microsoft Clarity und Meta Pixel werden erst nach Ihrer Einwilligung geladen. Der Google Tag Manager ist die übergeordnete technische Instanz für die drei darüber gesteuerten Services. Wenn Sie ihn deaktivieren, werden Google Analytics, Microsoft Clarity und Meta Pixel ebenfalls deaktiviert. Einzelheiten finden Sie in den folgenden Abschnitten.</p>`,
);

const tagManagerSection = numberedSection(
  "google-tag-manager",
  "Google Tag Manager",
  `<p>Der Google Tag Manager steuert auf dieser Website Google Analytics sowie die einwilligungsabhängige Auslösung von Microsoft Clarity und Meta Pixel. Anbieter für Personen im Europäischen Wirtschaftsraum ist Google Ireland Limited, Gordon House, Barrow Street, Dublin 4, Irland.</p>
<p>Wir verwenden den grundlegenden Einwilligungsmodus von Google. Vor Ihrer Einwilligung wird der Tag Manager vollständig blockiert; es wird weder das Tag-Manager-Skript abgerufen noch werden Einwilligungssignale oder Messdaten an Google übertragen. Wenn Sie Google Analytics, Microsoft Clarity oder Meta Pixel auswählen, wird der Tag Manager als erforderliche übergeordnete Instanz mit ausgewählt. Sie können ihn jederzeit wieder deaktivieren; dadurch werden alle drei abhängigen Services ebenfalls deaktiviert.</p>
...
const googleAnalyticsSection = numberedSection(
  "google-analytics",
  "Google Analytics",
  `<p>Google Analytics soll uns zeigen, wie häufig Seiten aufgerufen werden und wie Besucherinnen und Besucher durch das Angebot navigieren. Anbieter für Personen im Europäischen Wirtschaftsraum ist Google Ireland Limited, Gordon House, Barrow Street, Dublin 4, Irland.</p>
...
<p>Microsoft führt für Clarity die First-Party-Cookies <code>_clck</code> zur Speicherung einer pseudonymen Clarity-Nutzer-ID und von Website-Einstellungen sowie <code>_clsk</code> zur Verbindung mehrerer Seitenaufrufe zu einer Sitzung auf. Ihre Laufzeiten betragen bis zu ein Jahr beziehungsweise bis zu einen Tag. Zusätzlich nennt Microsoft die möglichen Third-Party-Cookies <code>CLID</code> zur Erkennung des erstmaligen Kontakts eines Browsers mit Clarity, <code>ANONCHK</code> zur Kennzeichnung einer möglichen Übertragung der Microsoft-Nutzer-ID MUID an das von Clarity nicht verwendete Werbe-Cookie ANID, <code>MR</code> zur Steuerung der MUID-Aktualisierung, <code>MUID</code> zur pseudonymen Browsererkennung auf Microsoft-Domains und <code>SM</code> zur Synchronisierung der MUID über Microsoft-Domains. Ob Third-Party-Cookies technisch gespeichert werden können, hängt auch von den Browser- und Cookie-Einstellungen ab. Microsoft nennt in der aktuellen Clarity-Cookie-Liste für diese Third-Party-Cookies keine festen Laufzeiten; sie werden durch Microsoft sowie die Browser- und Cookie-Einstellungen bestimmt. Weitere Einzelheiten enthält die <a href="https://learn.microsoft.com/en-us/clarity/setup-and-installation/clarity-cookies" rel="noopener noreferrer" target="_blank">Cookie-Liste von Microsoft Clarity</a>.</p>
...
<p><strong>Stand: 19. September 2026</strong></p>
...
<p>Für die technische Nutzungs- und Missbrauchsauswertung protokollieren wir diese Terminprüfungen für höchstens 30 Tage: Zeitpunkt, angefragte Daten, Ergebnis, HTTP-Status, Zugangsweg (zum Beispiel FAB oder API) sowie eine abgeleitete Browser-, Bot- oder Dienst-Kategorie mit Herkunftshinweis. Eine Prüfung gegen öffentlich veröffentlichte Anbieter-IP-Bereiche erfolgt auf unserem Server; dabei wird die Absender-IP nicht an den Anbieter übermittelt oder in diesem Protokoll gespeichert. Es werden keine zusätzlichen Tracking-Cookies gesetzt. Ergänzend speichern wir die grobe Geräteklasse (Desktop, Mobilgerät, Tablet oder unbekannt) und eine daraus abgeleitete Einschätzung der Bedienungsart. Dafür verwenden wir Browserangaben und einfache Signale zur Formularbedienung, ausdrücklich gemeldeter Automatisierung oder WebMCP-Nutzung; diese Signale werden ohne zusätzliche Kennungen oder Besucherprofile verarbeitet. Browserabfragen lassen nicht zuverlässig erkennen, ob ein Mensch oder ein KI-Assistent den Browser bedient. Die Auswertung zeigt Häufigkeiten je Wunschtermin, getrennt nach mutmaßlich manueller Nutzung, Bot-Hinweisen und unklaren Abrufen.</p>
...
<p>Um die Schnittstelle auf höchstens zwei erfolgreiche Abfragen innerhalb von 24 Stunden zu begrenzen, wird aus der beim Zugriff technisch übermittelten IP-Adresse unmittelbar mit einem geheimen Zusatz eine pseudonyme Kurzzeitkennung gebildet. Für diese Begrenzung werden nur die Kennung und die Zeitpunkte erfolgreicher Abfragen gespeichert. Einträge, deren 24-Stunden-Fenster abgelaufen ist, werden im nächsten regelmäßigen Bereinigungslauf gelöscht.</p>
<p>Zusätzlich protokollieren wir für höchstens 30 Tage den Zugangsweg, eine abgeleitete Bot- oder Dienst-Kategorie sowie die Art und das Ergebnis einer optionalen technischen Herkunftsprüfung, die konkret angefragten Wunschdaten, das jeweilige Verfügbarkeitsergebnis, den HTTP-Status und den Zeitpunkt der Anfrage. Die vollständige IP-Adresse, der vollständige User-Agent und ein optional übermittelter API-Schlüssel werden nicht gespeichert. Bot-Kennungen und vom Hosting-Proxy übermittelte Anbieter-IP-Adressen sind Herkunftshinweise, keine sichere Identifizierung einer Person oder eines KI-Modells. Diese Hinweise werden in der geschützten Ansicht entsprechend gekennzeichnet; ein individuell zugewiesener API-Schlüssel kann die zugehörige Client-Kennzeichnung bestätigen.</p>
<p>Die Verarbeitung dient dazu, konkrete Buchungsprüfungen nachzuvollziehen, die Schnittstelle technisch zu betreiben, Fehler zu erkennen und automatisierte Massenabfragen oder eine Umgehung der Abfragegrenze zu verhindern. Soweit die Anfrage auf einen konkreten Vertrag oder vorvertragliche Maßnahmen gerichtet ist, ist Art. 6 Abs. 1 lit. b DSGVO die Rechtsgrundlage. Im Übrigen erfolgt die Verarbeitung nach Art. 6 Abs. 1 lit. f DSGVO aufgrund unseres berechtigten Interesses an einer zuverlässigen, nachvollziehbaren und missbrauchsgeschützten Terminabfrage.</p>
